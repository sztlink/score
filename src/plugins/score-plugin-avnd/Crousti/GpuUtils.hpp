...
struct scene_outputs_storage<T>
{
  ...
  void upload(
      ...
      avnd::predicate_index<N>)
  {
    // Publish the scene every frame rather than only when `ctrl.dirty` is
    // set: a once-only push gets overwritten by any other producer on the
    // same downstream inlet. Consumers short-circuit on shared_ptr
    // identity + version, so publishing every frame costs only refcount
    // bumps.
    if(!ctrl.scene.state)
      return;
